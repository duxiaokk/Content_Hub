...
from ado_repost.main import run as run_once
from ado_repost.mempool import pool as mempool
...
def _make_idempotency_key(request: Request, body: dict[str, Any]) -> str:
    ...
    return raw if raw else "ado-repost:default"
...
def _maybe_register() -> None:
    ...
    base_url = (os.getenv("ADO_REPOST_BASE_URL") or "").strip().rstrip("/")
    if not scheduler_url or not base_url:
        return
    payload = {
        "agent_key": os.getenv("ADO_REPOST_AGENT_KEY", "ado-repost"),
        "name": os.getenv("ADO_REPOST_AGENT_NAME", "ado-repost"),
        "base_url": base_url,
        "task_types": ["ado_repost.run"],
        "health_path": "/health",
        "capabilities": {"kind": "ado_repost"},
        "status": 1,
    }
    try:
        httpx.post(
...
app = FastAPI(title="Ado Repost Agent", version="0.1.0", lifespan=lifespan)
...
def run_agent(request: Request, body: dict[str, Any]) -> dict[str, Any]:
    ...
    result_key = f"ado-repost:run-result:{idem_key}"
    lock_key = f"ado-repost:run-lock:{idem_key}"
