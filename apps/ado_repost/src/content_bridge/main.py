...
from content_bridge.fetchers import (
    CursorStore,
    FetchOrchestrator,
    FetchRequest,
    JsonCursorStore,
...
from content_bridge.publishing import BlogPublishingClient, DraftPayload, load_publishing_settings
from content_bridge.schema import DynamicItem
...
logger = logging.getLogger("content_bridge.main")
...
def run(
    ...
    if settings.fetchers.persist_cursors:
        if (
            os.getenv("CONTENT_BRIDGE_CURSOR_STORE", "").strip().lower() == "mempool"
            or os.getenv("ADO_REPOST_CURSOR_STORE", "").strip().lower() == "mempool"
        ):
    ...
    from content_bridge.processors import ContentProcessor
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Content bridge fetch and processing CLI")
