...
export default function PolishedReviewQueuePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : 'Failed to load review queue');
    } finally {
      setLoading(false);
    }
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : 'Failed to archive review');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <div>
      <div className="page-header">
        <div className="page-header__meta">
          <span className="page-header__eyebrow">Review Workflow</span>
          <Title level={3} className="page-header__title">
            审核队列
          </Title>
          <Text className="page-header__description">
            在这里完成内容审核、编辑与决策，通过后可直接发布到 Post。
          </Text>
        </div>
      ...
      <Card className="surface-card table-card" style={{ marginBottom: 16 }}>
        <Table
          rowKey="id"
          loading={loading}
          columns={columns}
      ...
      {!selected ? (
        ...
        <Row gutter={[16, 16]} align="stretch">
          <Col xs={24} xl={8}>
            <Card className="surface-card detail-card review-panel-card" title="原文">
          ...
          <Col xs={24} xl={8}>
            <Card
              className="surface-card detail-card review-panel-card"
              title="审核编辑"
              extra={
                <Space wrap>
            ...
            >
              ...
              {approveResult?.next_action === 'publish_to_post' ? (
                <Card size="small" className="review-next-card" style={{ marginTop: 16 }}>
          ...
          <Col xs={24} xl={8}>
            <Card className="surface-card detail-card review-panel-card" title="元数据">
              <Space direction="vertical" size={12} style={{ width: '100%' }}>
                ...
                <div>
                  ...
                  <div className="break-all">{selected.source_url || '-'}</div>
                </div>
