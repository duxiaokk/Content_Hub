...
- 2026-06-20: Code quality and documentation cleanup. Removed "初学者指南" boilerplate from 24 platform files, replaced verbose two-part docstrings with concise module descriptions. Deleted empty `agent.md` and outdated `.codex_prompt_template.md`. Moved `content_hub_端到端打通设计方案.md` to `infra/docs/`. Updated `shared_mempool_audit.md` with removal confirmation. Optimized `AGENTS.md` (removed obsolete references, consolidated rules). Added Node.js `engines` field to `frontend/package.json`. Added "Known Issues" section to `README.md`. Rebased Git history: renamed `1`/`2`/`重构`/`trae` commits to meaningful messages (`T000`/`T001-T007`/`T008-T019`/`T020`). `pytest` all core modules pass (74 tests). Force push pending (requires GitHub auth).
- 2026-06-20: Homepage redesign. Created `DashboardPage` as new workstation home with stat cards (today fetch count, pending review, published count, agent status), quick action buttons, and dual-column layout (recent fetch runs + recent published content). Extracted original `HomePage` article list to `PostListPage` at `/posts`. Reorganized `AppLayout` nav: workstation, sources, content library dropdown (content list, posts, fetch history), review queue, digests, task scheduling. Moved post routes: `/post/:id` → `/posts/:id`, `/create` → `/posts/new`. Added Node.js `engines` field to `frontend/package.json`. Frontend `vite build` passes successfully (no EPERM errors). `npx tsc -b` passes with zero type errors.
